...
export async function getAdminStats(): Promise<AdminStats> {
  ...
  const [
    ...
    supabase.from("waste_requests").select("*", { count: "exact", head: true }).eq("status", "collected"),
  ...
  return {
    ...
    collectedRequests: collected ?? 0,
    completedRequests: completedCount,
    // Platform revenue is ₵2.00 per completed job
    totalRevenue: completedCount * 2.00,
  };
}
